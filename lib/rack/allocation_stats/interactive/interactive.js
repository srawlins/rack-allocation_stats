...
function fillTableWith(groups, headers) {
  ...
  for (var tuple in groups) {
    ...
    for (var i in keys) {
      ...
      if (i === fileColonLineIndex) {
        ...
        tr.append($("<td>").text(classAbbreviations[keys[i]]));
      } else {
    ...
    if (showPercent) {
      ...
      if (pct < 10) { pct = " " + pct; }
      tr.append($("<td>").addClass("align-right").text(count + " (" + pct + "%)"));
    } else {
  ...
  $("<tr/>").
      ...
      append('<td class="align-right">' + totalCount + " (" + pct + "%)" + "</td>").
      appendTo($("#allocations tbody"));

  var filtered = allocations.length - totalCount;
  $("<tr/>").
      addClass("footer").
      append('<td class="align-right" colspan="' + (headers.length+1) + '">(' + filtered + ' more have been filtered out)</td>').
      appendTo($("#allocations tbody"));
...
function refillTable(e) {
  ...
  $.each(gbInputs, function(idx, gbInput) {
    if (gbInput.checked) {
      ...
      fields.push(field);
      if ((field === "class_plus" || field === "class") && abbreviateClassesInput.checked) {
        headers.push(field + " (abbrev)");
      } else {
        headers.push(field);
      }
    }
